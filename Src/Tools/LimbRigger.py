...
class LimbRigger:
    ...
    def __init__(self):
        ...
        self.controllerColorRGB = [1,1,1] #Defaullt controller color (white in RGB).
    ...
    def RigLimb(self):
        ...
        mc.setAttr(f"{poleVectorCtrlGrpName}.translate", poleVectorCtrlLoc.x, poleVectorCtrlLoc.y, poleVectorCtrlLoc.z, type="double3") #Set the position of the pole vector control.
        mc.poleVectorConstraint(poleVectorCtrlName, ikHandleName) #Constrain the IK Handle to the pole vector.

        mc.parent(ikHandleName, endIKCtrl) #Parent the IK handle under the IK controller.
        mc.setAttr(f"{ikHandleName}.v", 0) #Hide the IK handle.

        mc.connectAttr(f"{ikFkBlendController}. {ikFkBlendAttrName}", f"{ikHandleName}.ikBlend") #Connect the blend attribute to the IK handle.
        mc.connectAttr(f"{ikFkBlendController}. {ikFkBlendAttrName}", f"{endIKCtrlGrp}.v") #Control the IK controller's visibility.
        mc.connectAttr(f"{ikFkBlendController}. {ikFkBlendAttrName}", f"{poleVectorCtrlGrpName}.v") #Control the pole vector's visibility.

        reverseNodeName = f"{self.nameBase}_reverse" #Name for the reverse node in the node editor.
        mc.createNode("reverse", n=reverseNodeName) #Create the reverse node in the node editor.

        mc.connectAttr(f"{ikFkBlendController}. {ikFkBlendAttrName}", f"{reverseNodeName}.inputX") #Connect the blend attribute to the reverse node input.
        mc.connectAttr(f"{reverseNodeName}.outputX", f"{rootCtrlGrp}.v") #Use the reverse output to control the FK visibility.

        orientConstraint = None #Initialize the orient constraint variable.
        wristConnections = mc.listConnections(endJnt) #Get all the connections to the end joint.
        for connection in wristConnections: #Loop through the connections.
            if mc.objectType(connection) == "orientConstraint": #Check if it's an orient constraint.
                orientConstraint = connection #Store it
                break
        mc.connectAttr(f"{ikFkBlendController}.{ikFkBlendAttrName}", f"{orientConstraint}.{endIKCtrl}W1") #Connect the IK weight.
        mc.connectAttr(f"{reverseNodeName}.outputX", f"{orientConstraint}.{endCtrl}W0") #Connect the FK weight.

        topGrpName = f"{self.nameBase}_rig_grp" #Name for the top-level group.
        mc.group(n=topGrpName, empty=True) #Create an empty group.

        mc.parent(rootCtrlGrp, topGrpName) #Parent the FK controls under the top group.
        mc.parent(ikFkBlendCtrlControllerGrp, topGrpName) #Parent the blend control group.
        mc.parent(endIKCtrlGrp, topGrpName) #Parent the IK Control group.
        mc.parent(poleVectorCtrlGrpName, topGrpName) #Parent the pole vector group.

        #HW: Add color override for the topGrpName to be self.controllerColorRGB.
        mc.setAttr(f"{topGrpName}.overrideEnabled", 1) #Enable color override.
        mc.setAttr(f"{topGrpName}.overrideRGBColors", 1) #Use the RGB color mode.

        mc.setAttr(f"{topGrpName}.overrideColorR", self.controllerColorRGB[0]) #Set the red channel.
        mc.setAttr(f"{topGrpName}.overrideColorG", self.controllerColorRGB[1]) #Set the green channel.
        mc.setAttr(f"{topGrpName}.overrideColorB", self.controllerColorRGB[2]) #Set the blue channel.

class LimbRiggerWidget(MayaWidget):

    def __init__(self):
        super().__init__() #Initialize the parent widget.
        self.setWindowTitle("Limb Rigger") #Set the window title.
        self.rigger = LimbRigger() #Create the LimbRigger instance.
        self.masterLayout = QVBoxLayout() #This is the Main vertical layout.
        self.setLayout(self.masterLayout) #Assign the layout to the widget.
        self.controlColorRGB = [0,0,0] #Store the selected color.
        ...
        self.masterLayout.addLayout(self.infoLayout) #Add to the main layout.
        self.infoLayout.addWidget(QLabel("Name Base:")) #Label for name input.
        ...
        self.masterLayout.addWidget(QLabel("Base Color:")) #Label for color selection.

        self.controlColorBtn = QPushButton("Select Color") #Button to open the color picker.
        ...
        self.rigLimbBtn = QPushButton("Rig Limb") #Button to trigger the rigging.
    ...
    def SetNameBaseBtnClicked(self):
        self.rigger.SetNameBase(self.nameBaseLineEdit.text()) #Pass the text input to the rigger.

    def RigLimbBtnClicked(self):
        self.rigger.RigLimb() #Triggers the rigging process.
...
def Run():
    ...
    limbRiggerWidget.show() #Display the UI Window.
